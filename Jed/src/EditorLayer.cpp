...
void EditorLayer::OnAttach() {
  ...
  Jazz::FramebufferSpecification fbSpec;
  fbSpec.Width = 1280;
  fbSpec.Height = 720;
  m_Framebuffer = Jazz::Framebuffer::Create(fbSpec);
}
...
void EditorLayer::OnUpdate(Jazz::Timestep ts) {
  JZ_PROFILE_FUNCTION();

  // Resize
  if (Jazz::FramebufferSpecification spec = m_Framebuffer->GetSpecification();
...

  {
	...
	Jazz::Renderer2D::DrawQuad({0.5f, -0.5f}, {0.5f, 0.75f}, m_SquareColor);
...
void EditorLayer::OnImGuiRender() {
  ...
  if (ImGui::BeginMenuBar()) {
	if (ImGui::BeginMenu("File")) {
	  ...
	  if (ImGui::MenuItem("Exit")) Jazz::Application::Get().Close();
	  ImGui::EndMenu();
	}
...
void EditorLayer::OnEvent(Jazz::Event &e) {
